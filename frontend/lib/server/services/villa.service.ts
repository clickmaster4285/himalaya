import type { VillaCategory, VillaPublic } from "@/lib/server/data/villa-types";
import { VILLA_CATEGORIES } from "@/lib/server/data/villa-types";
import { VillaM } from "@/lib/server/models/schemas";
...
export function villaRowToPublic(row: VillaRow): VillaPublic {
  ...
  return {
    ...
    price: row.price,
    image: row.image,
    gallery: row.gallery?.length ? row.gallery : [],
