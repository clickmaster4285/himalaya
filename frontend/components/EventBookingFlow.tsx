...
const eventPackages = [
  ...
  },
  {
    ...
    tagline: "Great for medium events",
    guests: "50–100 Guests",
    price: 250_000,
    Icon: Star,
    features: [
      "Extended venue access",
      "Professional decoration & framing",
      "Advanced audio/visual system",
      "Premium catering (Meal + Beverages)",
      "Stage & podium setup",
      "Projector & screen",
      "Photography (4 hours)",
      "Welcome drink setup",
  ...
  },
  {
    ...
    tagline: "Luxury event experience",
    guests: "150–250 Guests",
    price: 450_000,
    Icon: Crown,
    features: [
      "Entire villa for 10 hours",
      "Luxury themed decoration",
      "Professional stage setup",
      "LED screens & AV equipment",
      "Photography & videography (full coverage)",
  ...
  },
  {
    ...
    Icon: Sparkles,
    features: [
      "Full resort access for 12 hours",
      "Royal themed decoration",
      "Celebrity host/performer (optional)",
      "Guest accommodation (20 rooms)",
      "Post-event highlight video",
      "Full AV & lighting production",
      "Dedicated event director",
      "Premium gifting & stationery",
    ],
  },
];

const addOnCatalog = [
  { id: "photo", name: "Extra Photography Hour", price: 15_000 },
  { id: "drone", name: "Drone Videography", price: 35_000 },
  { id: "booth", name: "Interactive Photo Booth", price: 25_000 },
  { id: "stream", name: "Live Streaming Service", price: 20_000 },
  { id: "host", name: "Celebrity Host / MC", price: 120_000 },
  { id: "band", name: "Live Music Band", price: 80_000 },
  { id: "dj", name: "Premium DJ Service", price: 45_000 },
  { id: "decor", name: "Enhanced Decoration", price: 55_000 },
];
...
export default function EventBookingFlow() {
  ...
  const [eventDate, setEventDate] = useState<Date | undefined>(defaultEventDate);
  const [startTime, setStartTime] = useState("09:00 AM");
  const [endTime, setEndTime] = useState("10:00 AM");
  const [selectedPkg, setSelectedPkg] = useState("basic");
  const [addonQty, setAddonQty] = useState<Record<string, number>>({ photo: 1, drone: 1 });
  const [paymentMethod, setPaymentMethod] = useState<"card" | "bank" | "jazzcash" | "easypaisa">("bank");

  const pkg = eventPackages.find((p) => p.id === selectedPkg) ?? eventPackages[0];
  ...
  const eventTotal = packagePrice + addOnTotal;
  const bookingRef = "HV-65893154";

  const dateLabel = formatShortDate(eventDate);
        ...
        {step !== 6 && (
          <>
            ...
            <p className="mt-3 text-[12px] md:text-[13px] text-neutral-500/90 max-w-2xl">
              Host your perfect event in our stunning Murree Hills venue.
            </p>
          </>
        )}
              ...
              >
                {step === 1 && (
                  <div className="p-8">
                    ...
                    <div className="mt-6">
                      <p className="text-[10px] uppercase tracking-[0.2em] text-neutral-500">Event type</p>
                      <select
                        value={eventType}
                ...
                {step === 3 && (
                  <div className="p-8 space-y-8">
                    <div>
                      <p className="font-display text-[15px] text-neutral-900">Organizer Information</p>
                    ...
                    <div className="mt-8 space-y-6 border-t border-[#eadfce] pt-6">
                      <div className="flex justify-between items-start">
                        <div className="flex gap-3">
                          <PartyPopper className="h-5 w-5 text-[#c9a55b]" />
                          <div>
                            <p className="text-[11px] uppercase tracking-widest text-neutral-500">Event Type</p>
                            <p className="mt-1 text-[14px] text-neutral-800">{eventType}</p>
                          </div>
                        </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex gap-3">
                          ...
                          <div>
                            <p className="text-[11px] uppercase tracking-widest text-neutral-500">Event Date &amp; Time</p>
                            <p className="mt-1 text-[14px] text-neutral-800">
                              {formatReviewDateTime(eventDate, startTime, endTime)}
                            </p>
                          </div>
                        </div>
                        <button type="button" className="text-[11px] text-[#c9a55b] underline">
                      ...
                      <div className="border border-[#eadfce] bg-[#fbf8f2] p-5">
                        <p className="text-[11px] uppercase tracking-widest text-neutral-500">Contact Information</p>
                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px] text-neutral-700">
                          <p>Full Name —</p>
                          <p>Organization —</p>
                          <p>Email —</p>
                          <p>Phone —</p>
                          <p className="sm:col-span-2">Expected Guests —</p>
                        </div>
                      </div>
              ...
              {step !== 6 && (
                <aside className="lg:col-span-4 space-y-6">
                  <div className="bg-[#1a1a1a] text-white p-7">
                    ...
                    <div className="mt-5">
                      <p className="text-[10px] uppercase tracking-[0.2em] text-[#c9a55b]/90">Event Type</p>
                      <p className="mt-2 text-[14px] font-semibold">{eventType}</p>
                    </div>
                    <div className="mt-5">
                    ...
                    {step >= 3 && selectedAddonLines.length > 0 && (
                      <div className="mt-5">
                        ...
                        <p className="mt-2 text-[12px] text-white/80">
                          {selectedAddonLines.map((a) => a.name).join(", ")}
                        </p>
                      </div>
                    )}
                    {step === 5 && (
                      <>
                        ...
                        <div className="mt-6 pt-4 border-t border-white/10 flex justify-between items-end">
                          ...
                          <span className="font-display text-2xl text-[#c9a55b]">PKR {eventTotal.toLocaleString()}</span>
                        </div>
                      </>
                    )}
                    {step < 5 && (
