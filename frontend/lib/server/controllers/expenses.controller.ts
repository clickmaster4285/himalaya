...
import { ExpenseM, BookingM, UserM } from "@/lib/server/models/schemas";
...
export async function listExpenses(req: Request, res: Response) {
  ...
  const bookingIds = [...new Set((rows as Array<Record<string, unknown>>).map((r) => String(r.bookingId)))];
  const userIds = [
    ...new Set(
  ...
  const bookingMap = new Map((bookings as Array<Record<string, unknown>>).map((b) => [String(b._id), b]));
  const userMap = new Map((users as Array<Record<string, unknown>>).map((u) => [String(u._id), u]));
  ...
  const expenses = (rows as Array<Record<string, unknown>>).map((r) => {
    ...
    return {
      ...
      updatedAt: r.updatedAt ? new Date(r.updatedAt as string | Date).toISOString() : null,
      booking: booking
        ? {
...
export async function createExpense(req: Request, res: Response) {
  ...
  const doc = await ExpenseM.create({
    bookingId,
    customerUserId: String((booking as Record<string, unknown>).userId),
    title,
