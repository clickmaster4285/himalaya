...
import { getPublishedVillasMerged, getVillaBySlugPublic } from "@/lib/server/data/villas-catalog";
import { getSessionFromRequest } from "@/lib/server/middleware/req-session";
import { isSuperAdmin } from "@/lib/server/utils/user-dto";
...
} from "@/lib/server/services/villa.service";
import { VILLA_CATEGORIES, type VillaCategory } from "@/lib/server/data/villa-types";
...
export async function listPublishedVillas(_req: Request, res: Response) {
  const villas = await getPublishedVillasMerged();
  return res.json({ villas });
}

export async function getPublishedVillaBySlug(req: Request, res: Response) {
...
export async function listAdminVillas(req: Request, res: Response) {
...
export async function getAdminVilla(req: Request, res: Response) {
  const session = await getSessionFromRequest(req);
  if (!session) return res.status(401).json({ error: "Unauthorized." });
  const user = await findUserById(session.userId);
  if (!user?.isActive || !isSuperAdmin(user.role)) {
...
export async function deleteVilla(req: Request, res: Response) {
  const session = await getSessionFromRequest(req);
  if (!session) return res.status(401).json({ error: "Unauthorized." });
  const user = await findUserById(session.userId);
  if (!user?.isActive || !isSuperAdmin(user.role)) {
